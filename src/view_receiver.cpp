...
void view_receiver_t::setup(void) {
...
bool view_receiver_t::loop(void) {
...
void view_receiver_t::close(void) {
    canvas.deleteSprite();
}

uint32_t view_receiver_t::getBarColor(int32_t y) {
    int32_t v  = 63 - (y >> 1);
    int32_t v2 = 63 - ((y + 1) >> 1);
    if ((v >> 4) != (v2 >> 4)) {
        return 0x887766u;
...
void view_receiver_t::fillBar(LovyanGFX* gfx, int32_t x, int32_t y, int32_t w,
                              int32_t h, size_t ch) {
...
void view_receiver_t::updateDisplay(int32_t duration) {
    timer += duration;
    int idx          = 0;
    ...
    for (int ch = 1; ch < DMX_MAX_PACKET_SIZE; ++ch) {
        ...
        if (graph_view) {
            ...
            int32_t x1 = static_cast<int32_t>(current_data);
            int32_t x2 = static_cast<int32_t>(prev_data);
            if (x1 > x2) {
                std::swap(x1, x2);
            }
            canvas.fillRectAlpha(32 + x1, canvas.height() - 1, x2 - x1 + 1, 1,
                                 128, bar_color_table[ch & 15]);
        } else {
            if (prev_data != current_data) {
                ...
                canvas.printf("%3d", ch);
                canvas.setTextColor(static_cast<uint8_t>(0x29u));
                canvas.print("]");
